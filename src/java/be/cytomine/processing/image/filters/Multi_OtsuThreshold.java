...
public class Multi_OtsuThreshold implements PlugInFilter
{
    ...
    public void run(ImageProcessor ip)
    {
        ...
        ///////////////////////////////////////////////////////////////
        if (gui) showRegions(MLEVEL, threshold, pixels, width, height);
        else result = getLastRegion(MLEVEL, threshold, pixels, width, height);
    }
    ...
    public ImagePlus getLastRegion(int mlevel, int [] t, byte [] pixels, int width, int height) {
        ImagePlus result = NewImage.createByteImage("OtsuResult", width, height,1, NewImage.FILL_BLACK);
        ImageProcessor ip = result.getProcessor();
        for (int i = 0; i < width*height; ++i) {
            int val = 0xff & pixels[i];
            if (val > t[mlevel-1]) ip.putPixel(i%width, i/width, val);
        }
        return result;
    }
