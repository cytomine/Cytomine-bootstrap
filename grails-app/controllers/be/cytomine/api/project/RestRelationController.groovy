...
class RestRelationController {
  ...
  def list = {
    ...
    def add = {
      ...
      if (result.status == 201) {
        ...
        new UndoStack(command : addRelationCommand, user: currentUser,transactionInProgress:  currentUser.transactionInProgress).save()
      }
  ...
  def update = {
    ...
    if (result.status == 200) {
      ...
      new UndoStack(command : editRelationCommand, user: currentUser, transactionInProgress:  currentUser.transactionInProgress).save()
    }
    }
  ...
  def delete =  {
    ...
    if (result.status == 204) {
      ...
      new UndoStack(command : deleteRelationCommand, user: currentUser, transactionInProgress:  currentUser.transactionInProgress).save()
    }
    response.status = result.status
    withFormat {
      json { render result.data as JSON }
